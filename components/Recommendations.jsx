...
import TutorialImg from "../public/static/tutorial.png";
import CPlogo from "../public/static/cp.jpg";
import Qazi from "../public/static/isaac.jpg";
import JSLogo from "../public/static/jsLogo.png";

const recommendedPost = [
  {
    title: "Did you know apples grow in kenyan Highlands,Read how to farm apples in kenya",
    image: ReplitLogo,
    author: {
      name: "Linda wangui",
      image: CPlogo,
  ...
  {
    title: "How to make more farming bulb onions",
    image: TutorialImg,
    author: {
      name: "Hamisi Boga",
      image: Qazi,
  ...
  {
    title: "How to become a successfull vegetable farmer",
    image: JSLogo,
    author: {
...
const Recommendations = () => {
  return (
    <div className="wrapper h-screen min-w-[10rem] max-w-[30rem] flex-[1.2] p-[2rem] ">
      <button className="accentedButton flex items-center justify-center text-sm bg-black text-white my-[2rem] py-[.6rem] rounded-full px-[8rem] ">
      ...
      <div className="searchBar flex items-center gap-[.6rem] h-[2.6rem] border px-[1rem] rounded-full ">
        ...
        <input
          type="text"
          className="searchInput border-none outline-none bg-none w-full "
          placeholder="Search"
        />
      </div>
      ...
      <div className="authorContainer my-[2rem]  ">
        ...
        <div className="authorName font-semibold mt-[.5rem] ">
          isaac kiplangat
        </div>
        <div className="authorFollowing font-semibold mb-[.2rem] text-[#787878] ">
          1M followers
        </div>
        <div className="authorActions flex gap-[.6rem] my-[1rem] ">
          <button className="actionButton bg-[#1A8917] text-white rounded-full px-[.6rem] py-[.4rem] text-sm ">
            follow
          </button>
          <button className="actionButton bg-[#1A8917] text-white rounded-full px-[.6rem] py-[.4rem] text-sm ">
            <MdMarkEmailRead />
      ...
      <div className="title pb-[.5rem] ">More from farmeet</div>
        ...
        <div className="articlesContainer">
          {recommendedPost.map((article, index) => {
            return (
              <div
                key={index}
                className="articlesContentWrapper flex items-center justify-between cursor-pointer my-[1rem]  "
              >
                <div className="articleContent flex-[4]">
                  <div className="recommendationAuthorContainer flex items-center gap-[.6rem]  ">
                    ...
                    <div className="recommendationAuthorName text-sm ">
                      {article.author.name}
                    </div>
                  </div>
                  <div className="recommendationTitle font-bold ">
                    {article.title}
                  </div>
                </div>
                ...
                <div className="recommendationThumbnailContainer flex flex-1 items-center justify-center h-[4rem] w-[4rem] overflow-hidden ">
                  <Image
                    className="recommendationThumbnai"
                    objectFit="cover"
                    src={article.image}
                    height={100}
                    width={100}
