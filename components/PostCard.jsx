...
const PostCard = ({ post }) => {
  ...
  useEffect(() => {
    const getAuthorData = async () => {
      setAuthorData((await getDoc(doc(db, 'users', post.author))).data())
    }
    getAuthorData()
  }, [])
      ...
      <div className="wrapper flex max-w-[46rem] h-[10rem] items-center gap-[1rem] cursor-pointer ">
        <div className="postDetails flex flex-[2.5] flex-col  ">
          <div className="authorContainer flex gap-[.4rem] ">
            <div className="authorImageContainer grid place-items-center rounded-full overflow-hidden h-[1.4rem] w-[1.4rem] ">
              <Image
                width={40}
                height={40}
          ...
          <div className="detailsContainer flex items-center justify-between text-[#787878] ">
            <span className="articlesDetails my-2 text-[.8rem]  ">
              ...
              • {post.postLength} min read •{' '}
              <span className="category bg-[#F2F3F2] p-1 rounded-full ">
                {post.category}
              </span>
            </span>
            <span className="bookmarkContainer cursor-pointer ">
        ...
        <div className="thumbnailContainer flex-1 ">
          <Image
            ...
            className="thumbnail"
            objectFit="cover"
            src={`https://res.cloudinary.com/demo/image/fetch/${post.bannerImage}`}
            alt="logo"
          />
        </div>
      </div>
